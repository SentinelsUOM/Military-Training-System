...
🆕 **Read the third column carefully: it justifies each state's *rank*, not its number.** The percentages themselves are the project's calibration — see the note under the table.

| State | Distress % *(calibrated)* | Why it ranks here *(research)* |
|---|---:|---|
...
🆕 **What is and is not research-derived here — state this plainly in the dissertation.** The **ordering** is research-derived, including both counter-intuitive rankings (Freeze above Panic; Threatened above Panic). The **scale type** — a 0–100 subjective-distress index — is modelled on the validated **SUDS** instrument. The **specific percentages are the project's own calibration and must not be attributed to any paper.** SUDS is a *self-report* instrument: a person rates their own distress 0–100, so no published table maps a categorical state onto a number, and nothing in the literature says "Freeze = 72 %". The Expert Value Table described `0/10/33/50/66` as *"a reasonable compressed version"* of SUDS — an endorsement of plausibility, not a derivation. `Held 40`, `Threatened 75`, `Wounded 95`, `Down 100` and the revised `Freeze 72` are calibrations the project chose; the research fixes only where they sit *relative to each other*. This is the same standing as the Speed coefficients (§4.8) and `threatDiscrimination` (§6.5b) — see `MODULE4_EVALUATION.md` §1 for the full statement.

The Freeze-above-Panic ordering is the one deliberate research-driven correction to an otherwise intuitive "louder/closer = worse" scale, and is the most strongly validated element of the distress model (§6.1). The sound/emotion research behind the full state model — gunfire sound-pressure level as a function of distance, the inverse-square propagation law, the Bracha (2004) five-stage acute-stress-response spectrum, and why a scream or vocal threat outranks a same-loudness gunshot — is compiled in `HOSTAGE_EMOTION_SOUND_RESEARCH.pdf`, with six original supporting charts safe for submission (not reproductions of copyrighted journal figures) in `Module4_Research_Figures.docx.pdf`. Note the module-boundary honesty point: the *sound-driven triggering* of state transitions is implemented in Module 2's `HostageController.cs`; Module 4 owns the resulting *distress-index scoring and visualisation* of whatever state the hostage is in.
